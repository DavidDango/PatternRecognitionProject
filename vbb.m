...
function A = rebuildAll()

currdir = uigetdir; %gets directory
files = dir(fullfile(currdir,'*.vbb')); %gets all wav files in struct
for i = 1:length(files)
  A = vbb('vbbLoad', strcat(currdir, '\', files(i).name));
  vbb('vbbSaveTxt', A, strcat(currdir, '\', strrep(files(i).name,'.vbb',''), '.txt'));
end
